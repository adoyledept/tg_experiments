...
# to all production account and environments (production, failover)
...
terraform {
  ...
  before_hook "before_plan_apply_hook" {
    commands = ["plan", "apply"]
    execute  = ["echo", "START Delve.bio PROD Terragrunt execution"]
  }
  after_hook "after_plan_apply_hook" {
    commands = ["plan", "apply"]
    execute  = ["echo", "FINISH Delve.bio PROD Terragrunt execution"]
  }

  before_hook "before_destroy_hook" {
    commands = ["destroy"]
    execute  = ["echo", "START Delve.bio PROD Terragrunt destroy"]
  }
  after_hook "after_destroy_hook" {
    commands = ["destroy"]
    execute  = ["echo", "FINISH Delve.bio PROD Terragrunt destroy"]
  }
